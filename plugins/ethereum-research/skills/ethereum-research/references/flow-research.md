...
Before any research, verify that submodules are available in the user project root.

Run the following commands:

```
ls .ethereum-research/go-ethereum/core/ 2>/dev/null && echo "go-ethereum: OK" || echo "go-ethereum: MISSING"
ls .ethereum-research/forkcast/ 2>/dev/null && echo "forkcast: OK" || echo "forkcast: MISSING"
ls .ethereum-research/EIPs/EIPS/ 2>/dev/null && echo "EIPs: OK" || echo "EIPs: MISSING"
```
...
```
git -C .ethereum-research/go-ethereum log --oneline -10
```

forkcast recent changes:

```
git -C .ethereum-research/forkcast log --oneline -10
```

EIPs recent changes:

```
git -C .ethereum-research/EIPs log --oneline -10
```
...
|---------------|----------------|-------------------|
| Protocol or EVM internals | go-ethereum code, EIPs | ethresear.ch |
| EIP analysis | EIPs repo, ethresear.ch | Vitalik blog |
| Hardfork tracking | forkcast, go-ethereum | Ethereum blog |
| General ecosystem | all web sources, relevant repos | -- |
...
| Community level | ethresear.ch threads, blog posts, Vitalik writings, open discussions |
