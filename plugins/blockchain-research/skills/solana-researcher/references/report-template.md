---
title: Solana Research Report Output Template
impact: CRITICAL
impactDescription: Every research report must be emitted in both Markdown and HTML using the plugin-level shared template
tags: report, template, output, solana, markdown, html, mermaid, svg
---

# Research Report Output -- Solana
...
`{slug}` is kebab-case, derived from the report title (for example `gettransaction-field-decomposition`).
...
Every Solana research report must contain **at minimum**:

- One Mermaid flowchart showing how raw fields (block, transaction message, instructions, innerInstructions, logMessages, pre/postBalances, pre/postTokenBalances) flow into derived views used by an indexer.
- One Mermaid sequence diagram showing the time-ordered actor interaction relevant to the topic (for example: payer -> runtime -> program -> recipient, including CPI calls).
- One Mermaid state diagram when state transitions are in scope (for example: commitment level progression, log FSM for parsing program logs).
- At least one inline SVG diagram for content Mermaid cannot express cleanly (byte-offset maps of an Anchor event body, account role quadrants from transaction header, stackHeight -> tree reconstruction, effective_keys layout with static + ALT).
- Field-by-field tables for raw response decomposition.
- Callouts for invariants and version-specific caveats (for example, legacy vs v0 transaction differences).
- A parsing / indexing checklist at the end.

## Required Sections

The shared template hard-codes the section order. Keep it intact. Required sections for a Solana report:
...
5. Protocol-level analysis (SIMDs, consensus rules, runtime behavior)
6. Code-level analysis (solana / agave / SPL file paths and line references)
7. On-chain data view (fields, records, balance deltas, log semantics, CU / cost breakdown)
...
13. Footer (generation date, source skill = `blockchain-research:solana-researcher`)
...
- Use tables for SIMD listings, feature-activation listings, and field-by-field decomposition.
...
## Solana-Specific References Section

The References section of a Solana report should always include, when relevant:

- Local source paths in `<RESEARCH_ROOT>/solana/...`, `<RESEARCH_ROOT>/agave/...`, `<RESEARCH_ROOT>/solana-program-library/...`.
- SIMD links: `https://github.com/solana-foundation/solana-improvement-documents/blob/main/proposals/XXXX.md`.
- Solana docs: https://solana.com/docs
- forum.solana.com threads when applicable.
- Solana blog and Anza engineering blog posts when applicable.
- Anchor docs when the topic involves Anchor programs or IDL decoding.
